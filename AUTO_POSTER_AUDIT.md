...
## Deploy Readiness Checklist Completed (2026-06-30)
...
|---------|--------|
| `npm run build` | Pass — syntax checks + EJS compile + Vite build |
| `npm test` | Pass — 51 tests, 0 failures, ~1.9s |
| `git log --oneline -4` | `6d92cac`, `edb8e87`, `1d9f880`, `584326e` (4 fix commits across P0+P1 loops) |

### No source files changed in this loop

This loop is documentation-only. No source code, tests, or config files were modified.

### 1. Deployment Environment Checklist

All required environment variables for Render deployment. Values are placeholders only — no real secrets documented.

#### Critical (app won't start without these)

| Env Var | Purpose | Set on Render |
|---------|---------|---------------|
| `ADMIN_PASSWORD` | Admin login password (>=12 chars) | Web service, `sync: false` |
| `FIREBASE_PROJECT_ID` | Firebase project ID | Web service |
| `FIREBASE_CLIENT_EMAIL` | Firebase service account email | Web service |
| `FIREBASE_PRIVATE_KEY` | Firebase service account private key (PEM, `\n` escaped) | Web service |
| `CRON_SECRET` | Shared secret between web and cron services | Blueprint env group `chanter-scheduler-shared` |

#### Required for TikTok posting

| Env Var | Purpose |
|---------|---------|
| `TIKTOK_CLIENT_KEY` | TikTok app client key |
| `TIKTOK_CLIENT_SECRET` | TikTok app client secret |
| `TIKTOK_REDIRECT_URI` | OAuth callback: `https://poster.chanterr.com/auth/tiktok/callback` |
| `TIKTOK_PRIVACY_LEVEL` | Default `SELF_ONLY` — posts are private until approved |
...
#### Required for cron/scheduler

| Env Var | Purpose |
|---------|---------|
| `APP_URL` | Web service HTTPS URL (e.g., `https://poster.chanterr.com`) — set on cron service |
| `APP_TIME_ZONE` | `Asia/Nicosia` — set in Blueprint env group |
| `TZ` | `Asia/Nicosia` — set in Blueprint env group |

#### Recommended security

| Env Var | Purpose |
|---------|---------|
| `ADMIN_SESSION_SECRET` | Independent session signing key (auto-generated by Render Blueprint) |
| `ENABLE_INSTAGRAM` | `false` — keep Instagram disabled until Meta app review |

#### Optional (auto-caption / auto-music)

| Env Var | Purpose |
|---------|---------|
| `AI_PROVIDER` | `gemini` (default) |
| `GEMINI_API_KEY` | For auto-caption generation |
| `OPENAI_API_KEY` | For auto-caption + audio transcription |
| `QWEN_API_KEY` | For auto-caption fallback provider |

#### Scheduler tuning (optional, defaults are safe)

| Env Var | Default | Purpose |
|---------|---------|---------|
| `SCHEDULER_STALE_LOCK_MINUTES` | `20` | How long before a processing job is reclaimed |
| `SCHEDULER_MAX_CLAIM_ATTEMPTS` | `5` | Max retries before marking a job failed |

### 2. Render Setup Checklist

#### Web Service

- [ ] **Type:** Web Service (Node.js)
- [ ] **Plan:** `starter` (acceptable for private test) or `standard` (recommended for production — no sleep)
- [ ] **Build:** `npm ci && npm run build`
- [ ] **Start:** `npm start`
- [ ] **Health check:** `/health`
- [ ] **Auto-deploy:** Enabled (push to `main` triggers deploy)
- [ ] **Node version:** 22
- [ ] All Critical + Required env vars set
...
- [ ] **Schedule:** `* * * * *` (every minute)
- [ ] **Build:** `npm ci`
- [ ] **Start:** `npm run scheduler:ping`
- [ ] **Node version:** 22
- [ ] **`APP_URL`:** Set to web service HTTPS URL
- [ ] **`CRON_SECRET`:** Same value as web service (via Blueprint env group)

#### Post-deploy verification

- [ ] `GET /health` returns `ok: true` with `schedulerHealth.lastTickAt` populated within 2 minutes
- [ ] Render cron job logs show `[scheduler-ping]` entries
- [ ] Web service logs show `[CRON_TICK]` entries

#### Free-tier sleep risk

- **Starter plan** services sleep after ~15 min of inactivity. The every-minute cron ping prevents this under normal operation.
- **If the cron job itself fails**, the web service will sleep and scheduled posts will be delayed (not lost — Firestore persists them). They will be published on the next successful tick.
- **Recommendation:** Upgrade to `standard` plan for production to eliminate sleep risk entirely.

#### Expected behavior after restart

- Pending/scheduled jobs in Firestore are untouched
- First cron tick after boot runs `reclaimStaleLocks()` -> recovers any jobs stuck in `processing`
- `getSchedulerHealth().lastTickAt` will be `null` until the first tick completes
- `getSchedulerHealth().staleProcessingCount` may be >0 if there were crashed jobs — should drop to 0 after the first tick

### 3. Private Production Smoke Test Plan

Execute these steps manually after deployment. Steps marked with WARNING can trigger a real TikTok post and require human confirmation.

#### Phase 1: Infrastructure verification (safe)

1. [ ] Deploy to Render
2. [ ] Wait 2 minutes, then `GET /health` — confirm `ok: true`, `schedulerHealth.lastTickAt` is recent
3. [ ] Confirm `schedulerHealth.staleProcessingCount === 0`
4. [ ] Confirm `schedulerHealth.stuckPendingCount === 0`
5. [ ] Open dashboard at `/private/autoposter` — login with admin password
6. [ ] Confirm no TikTok account is connected yet

#### Phase 2: TikTok OAuth (safe, no posting)

7. [ ] Click "Connect TikTok" — complete OAuth flow
8. [ ] Confirm dashboard shows connected account with username
9. [ ] Confirm `GET /health` still returns `ok: true`

#### Phase 3: Token rotation (safe, no posting)

10. [ ] Click "Disconnect TikTok" — confirm account shows disconnected
11. [ ] Click "Connect TikTok" again — get a fresh token (this invalidates any old tokens from git history)
12. [ ] Confirm dashboard shows connected account

#### Phase 4: Upload and schedule (safe, no posting yet)

13. [ ] Upload a test image or short video
14. [ ] Set caption to "TEST POST - DO NOT PUBLISH"
15. [ ] Confirm `TIKTOK_PRIVACY_LEVEL` is `SELF_ONLY`
16. [ ] Schedule the post for 3 minutes from now
17. [ ] Confirm dashboard shows status "Scheduled" with correct time
18. [ ] `GET /api/debug/jobs` (with cron secret) — confirm job appears with `status: scheduled`

#### Phase 5: Scheduled publish (WARNING: CAN TRIGGER REAL TIKTOK POST)

19. [ ] WARNING: **Human confirmation required:** You are about to publish to TikTok. The post will be `SELF_ONLY` (private) but it will appear on the connected TikTok account.
20. [ ] Wait for the scheduled time to pass
21. [ ] Wait 2 more minutes for the cron tick to fire
22. [ ] Check dashboard — job status should change to "Posted"
23. [ ] Check `GET /api/debug/jobs` — confirm `status: posted`, `publishId` is set
24. [ ] Verify on TikTok: the post appears as a private/draft post

#### Phase 6: Duplicate-post guard verification (safe)

25. [ ] Try to manually "Post Now" on the same completed job
26. [ ] Confirm it is blocked ("skipped" — job already has `publishId`)
27. [ ] Try to force-process via API — confirm still blocked

#### Phase 7: Failed-job recovery (safe, no posting)

28. [ ] Upload another test image, schedule for 2 min from now
29. [ ] Before the tick fires, manually set the job status to `processing` in Firestore (simulating a crash)
30. [ ] Wait 22+ minutes (exceed `SCHEDULER_STALE_LOCK_MINUTES`)
31. [ ] Confirm `reclaimStaleLocks()` recovers the job — status returns to `scheduled`
32. [ ] Confirm the next tick publishes it successfully
...
1. [ ] In Render dashboard -> Web Service -> Settings -> set `AUTO_DEPLOY` to disabled
2. [ ] In Render dashboard -> Cron Job -> disable the cron job
3. [ ] In Render dashboard -> Web Service -> Deploy -> rollback to previous deploy (`c2cf77b` or latest known-good commit)
4. [ ] Confirm `/health` responds with the old version

#### TikTok token revocation

5. [ ] In the dashboard, click "Disconnect TikTok" to clear tokens
6. [ ] This prevents any scheduled posts from firing

#### Data safety

7. [ ] Firestore data is preserved across deploys — no data loss on rollback
8. [ ] Cloudinary media is preserved — no media loss on rollback
9. [ ] Jobs in `scheduled` or `pending` status will remain queued but won't fire until the cron job is re-enabled

#### Rollback commit reference

| Version | Commit | Notes |
|---------|--------|-------|
| Pre-audit baseline | `c2cf77b` | Last commit before P0/P1 hardening |
| P0 complete | `584326e` | CSRF + git hygiene + claimAttempts reset |
| P1 Security | `1d9f880` | scrypt + env validation + token redaction |
| P1 Ledger | `edb8e87` | publishId + duplicate guard + redacted errors |
| P1 Reliability | `6d92cac` | health endpoint + stale lock monitoring |

### 5. Final Status

**Readiness label:** Private Production MVP Candidate

**Not SaaS-ready.** The following blockers prevent SaaS/multi-tenant use:
- Single admin password (no multi-user auth)
- In-memory login rate limiting (P1-2)
- No session revocation (P2-6)
- No CI/CD pipeline (P2-7)
- No Firestore backup strategy (P2-4)
- No structured logging (P2-1)
- No request rate limiting on API endpoints (P2-2)

**Safe for:** Single-operator private TikTok scheduling with `SELF_ONLY` privacy level.

**Not safe for:** Multi-user, public-facing, or automated production without human oversight.
...
- P2-1: Structured logging (pino/winston)
- P2-2: Request rate limiting on non-auth endpoints
- P2-3: Cloudinary health in `/health`
- P2-4: Firestore backup/restore strategy
- P2-5: Legacy `findDueJobs()` dual query cleanup
- P2-6: Session revocation mechanism
- P2-7: CI/CD pipeline

#### Next Recommended Human Actions

1. **Deploy to Render** following the Render Setup Checklist above
2. **Complete the Private Production Smoke Test** (Phases 1-7)
3. **Rotate TikTok OAuth tokens** by disconnecting and reconnecting (Phase 3 above)
4. **Set up external uptime monitoring** on `GET /health` — alert if `lastTickAt` >5 min stale
5. **Consider upgrading Render plan** from `starter` to `standard` to eliminate sleep risk
6. **Review remaining P1-2** (login rate limiting) when ready for next hardening sprint

### Explicit Note

**No real TikTok post was triggered** during this loop. This is a documentation-only loop. All verification was done via `npm test` (51/51 pass) and `npm run build` (pass). No live TikTok API endpoints were called.

### Git Commit Summary (all loops)

| Commit | Loop | Description |
|--------|------|-------------|
| `584326e` | P0 | CSRF protection + git hygiene + claimAttempts reset |
| `1d9f880` | P1 Security | scrypt password hashing + env validation + token redaction |
| `edb8e87` | P1 Ledger | publishId storage + duplicate-post guard + redacted error metadata |
| `6d92cac` | P1 Reliability | health endpoint enhancement + stale lock monitoring + lastTickAt tracking |
